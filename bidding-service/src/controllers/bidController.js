...
const { Bid, pool } = require("../models/bidModel");
...
async function handleBidProcessing({ auction_id, supplier_id, price }) {
  try {
    ...
    if (Object.keys(auction).length === 0) {
      ...
      auction = {
        ...
        extension_duration_minutes:
          dbAuction.extension_duration_minutes.toString(),
    ...
    const currentL1 = await redisClient.zRangeWithScores(
      `auction:${auction_id}:bids`,
      0,
      0,
    );
    if (currentL1.length > 0 && price >= currentL1[0].score) {
      ...
      io.to(`user:${supplier_id}`).emit("bid_error", {
        auction_id,
        message: `Your bid $${price} did not beat the current L1 of $${currentL1[0].score}`,
      });
    ...
    const triggerWindowMs =
      parseInt(auction.trigger_window_minutes) * 60 * 1000;
    const extensionMs =
      parseInt(auction.extension_duration_minutes) * 60 * 1000;
      ...
      if (wouldTrigger) {
        ...
        if (!canExtend) {
          ...
          io.to(`user:${supplier_id}`).emit("bid_error", {
            auction_id,
            message:
              "Bid rejected: Cannot extend auction beyond forced close time",
          });
    ...
    const oldRankings = await redisClient.zRange(
      `auction:${auction_id}:bids`,
      0,
      -1,
    );
    const oldSupplierRank = oldRankings.indexOf(supplier_id.toString());
    await redisClient.zAdd(`auction:${auction_id}:bids`, {
      score: price,
      value: supplier_id.toString(),
    });

    const newRankings = await redisClient.zRange(
      `auction:${auction_id}:bids`,
      0,
      -1,
    );
    const newL1 = await redisClient.zRangeWithScores(
      `auction:${auction_id}:bids`,
      0,
      0,
    );
    ...
    if (bidCloseTime - now <= triggerWindowMs) {
      ...
      if (auction.trigger_type === "ANY_BID") shouldExtend = true;
        ...
        auction.trigger_type === "ANY_RANK_CHANGE" &&
        (oldSupplierRank === -1 || oldSupplierRank !== newSupplierRank)
      )
        shouldExtend = true;
      else if (
        auction.trigger_type === "L1_CHANGE" &&
        (currentL1.length === 0 || currentL1[0].value !== newL1[0].value)
      )
        shouldExtend = true;
      ...
      if (shouldExtend) {
        ...
        if (newCloseTime > bidCloseTime) {
          ...
          await redisClient.hSet(
            `auction:${auction_id}:metadata`,
            "bid_close_time",
            auction.bid_close_time,
          );
    ...
    // Fetch username for better UX and logging
    const userRes = await pool.query("SELECT username FROM users WHERE id = $1", [supplier_id]);
    const supplier_name = userRes.rows[0]?.username || `Supplier #${supplier_id}`;

    console.log(`[Bidding-Service] Processed bid: Auction=${auction_id}, Supplier=${supplier_name} (${supplier_id}), Price=$${price}`);
    ...
    const bidEvent = {
      ...bidToLog,
      bid_close_time: auction.bid_close_time,
      extended,
      rankings: newRankings,
    };
    io.to(`auction:${auction_id}`).emit("bid_update", bidEvent);
    if (extended) {
      io.to(`auction:${auction_id}`).emit("auction_extended", {
        auction_id,
        new_close_time: auction.bid_close_time,
      });
    }
...
exports.placeBid = async (req, res) => {
  ...
  if (!auction_id || !price)
    return res
      .status(400)
      .json({ message: "auction_id and price are required" });
  console.log(
    `Received bid from supplier ${supplier_id} for auction ${auction_id} with price $${price}`,
  );
...
exports.getBids = async (req, res) => {
  ...
  try {
    ...
    const rankings = await redisClient.zRange(
      `auction:${auction_id}:bids`,
      0,
      -1,
    );
    console.log(`Fetched history and rankings for auction ${auction_id}`);
