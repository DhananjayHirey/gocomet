...
import { Gavel, User, LogOut, Plus, Clock, Timer, Zap, BarChart3, ChevronRight } from 'lucide-react';
import { formatDistanceToNow } from 'date-fns';
import { Topbar } from '../shared/UIComponents';
import { statusConfig } from '../../constants/status';

const Dashboard = ({ user, auctions, loading, onSelectAuction, onCreateClick, onLogout }) => {
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '48px 32px' }}>
        <div style={{ marginBottom: 48, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 20 }}>
          <div>
            ...
            <p style={{ color: 'var(--text-2)', marginTop: 8, fontSize: 15 }}>{auctions.length} RFQ{auctions.length !== 1 ? 's' : ''} available</p>
          </div>
          {user?.role === 'BUYER' && (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 20 }}>
              ...
              return (
                <motion.div key={auction.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06, duration: 0.35, ease: [0.22, 1, 0.36, 1] }}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      {status.dot && <div className="pulse-dot" style={{ width: 6, height: 6 }} />}
                      <span className={`tag ${ status.cls }`}>{status.label}</span>
                    </div>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--ink-3)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <Gavel size={16} color="var(--text-3)" />
                    </div>
                  </div>
                  <h3 className="syne" style={{ fontSize: 18, fontWeight: 700, color: 'var(--text)', marginBottom: 16, lineHeight: 1.3, letterSpacing: '-0.01em' }}>{auction.name}</h3>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--text-2)', fontSize: 13 }}>
                      <Clock size={14} color="var(--text-3)" />
                      <span>Closes <span style={{ color: 'var(--text)' }}>{formatDistanceToNow(new Date(auction.bid_close_time), { addSuffix: true })}</span></span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--text-2)', fontSize: 13 }}>
                      <Timer size={14} color="var(--text-3)" />
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--text-2)', fontSize: 13 }}>
                      ...
                      <span className="mono" style={{ fontSize: 12, color: 'var(--accent-2)', background: 'rgba(123,97,255,0.08)', padding: '2px 8px', borderRadius: 6 }}>{auction.trigger_type}</span>
                    </div>
                  </div>
            ...
            {auctions.length === 0 && (
              <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '80px 20px', color: 'var(--text-3)' }}>
                ...
                <p className="syne" style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>No auctions yet</p>
