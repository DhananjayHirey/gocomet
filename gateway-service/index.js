...
const AUCTION_SERVICE_URL =
  process.env.AUCTION_SERVICE_URL || "http://auction-service:3002";
const BIDDING_SERVICE_URL =
  process.env.BIDDING_SERVICE_URL || "http://bidding-service:3003";
...
app.post("/bid", express.json(), async (req, res) => {
  ...
  try {
    ...
    console.log("decoded token in gateway-service: ", decoded);
    console.log("decoded id in gateway-service: ", decoded.id);
    ...
    console.log(`[Gateway] Placing bid: User=${decoded.id}, Auction=${auction_id}, Price=${price}`);
